...
app.controller('StorageAmendmentToConsignmentAgreementController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    ...
    if ($stateParams.product_quote_agreement_id)
    {
        ...
        $http.post(site_settings.api_url + 'consignment_agreement_with_storage/check_agreement_storage_amendment', {'product_quote_agreement_id': $stateParams.product_quote_agreement_id})
                .then(function (response)
                {
                    ...
                    { 
                       if ($scope.response.status)
                        {
                            $state.go('storage_amendment_to_consignment_agreement_completed', {status: 'already_submitted'});
                        }

                    } else
                    {
                        $state.go('storage_amendment_to_consignment_agreement_completed', {status: 'invalid'});
                    }
    ...
    if ($stateParams.product_quote_agreement_id)
    {
        $scope.SaveSellerAgreement = function (is_valid)
        {
            if (is_valid)
            {
                ...
                $http.post(site_settings.api_url + 'consignment_agreement_with_storage/save_agreement_storage_amendment', {'seller_ageement': $scope.seller_agreement, 'signature': $scope.signature})
                        .then(function (response)
                        {
                            $scope.response = response.data;
...
app.controller('StorageAmendmentToConsignmentAgreementCompletedController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    $scope.title = '';
    $scope.message = '';
