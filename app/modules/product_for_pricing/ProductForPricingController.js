...
app.controller('ProductForPricingController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder, ngAAToken, $window)
{
...
//                        $scope.downloadDocument();
    ...
    $scope.sendMailApprove = function ()
    {
        ...
        if (approved.length > 0)
        {
            ...
            {
                $rootScope.loader = true;
                $http.post(site_settings.api_url + 'product_for_pricing/send_mail_approve', {products: approved, seller: $stateParams.id})
                        .then(function (response)
                        {
...
//                        $scope.downloadDocument();
    ...
    $scope.changeAcceptProductForProductionStatus = function ()
    {
        ...
        if (rejected.length > 0)
        {
            ...
            {
                $rootScope.loader = true;
                $http.post(site_settings.api_url + 'product_for_pricing/change_product_for_pricing_status', {product_status: rejected})
                        .then(function (response)
                        {

                            $rootScope.loader = false;
                            $scope.productStatus = [];
                            $rootScope.message = 'Product Status Change Successfully';
                            $rootScope.$emit("notification");
                            $rootScope.$emit("reloadUserTable");
                            $rootScope.getPendingProducts();
//                        $scope.downloadDocument();
    ...
    $scope.changeStoragePoposalStatus = function ()
    {
        var rejected = [];
        for (var i in $scope.productStatus)
        {
        ...
        {
            $rootScope.message = 'Please select at least one Storage price';
            $rootScope.$emit("notification");
        }
    };
...
//            });
    ...
    {
        $mdDialog.show({
            controller: 'ProductForPricingAddController',
            templateUrl: 'app/modules/product_for_pricing/views/product_for_pricing_add.html',
            parent: angular.element($document.body),
            clickOutsideToClose: true,
            locals: {
    ...
    $scope.openProductQuotationPricerDialog = function (product_quotation)
    {
        $mdDialog.show({
            ...
            templateUrl: 'app/modules/product_for_pricing/views/product_for_pricing_edit_pricer.html',
            parent: angular.element($document.body),
            clickOutsideToClose: true,
            locals: {
                product_quotation: product_quotation
    ...
    $scope.openProductStoragepriceAddDialog = function (product_quotation)
    {
        $mdDialog.show({
            controller: 'ProductForStoragePricingAddController',
            templateUrl: 'app/modules/product_for_pricing/views/product_for_storage_pricing_add.html',
            parent: angular.element($document.body),
            clickOutsideToClose: true,
            locals: {
                product_quotation: product_quotation
...
//         $window.location.reload();
    ...
    $scope.sendPricingProposalMail = function ()
    {
        ...
        if (approved.length > 0)
        {
            ...
            var confirm = $mdDialog.confirm()
                    .title('Are you sure you want to Send Pricing Proposal?')
                    .ok('Yes')
                    .cancel('No');
...
//                        $scope.downloadDocument();
    ...
    $scope.saveProductStatusRejected = function ()
    {
        var rejected = [];
        for (var i = 0; i < $scope.productStatus.length; i++)
        {
...
//                        $scope.downloadDocument();
    ...
    $scope.assignToAgent = function ()
    {
        ...
        if (rejected.length > 0)
        {
            ...
            $mdDialog.show({
                controller: 'AssignAgentToProductQuotationController',
    ...
    $scope.generatePricingReport = function () {
        $rootScope.loader = true;
        $http.post(site_settings.api_url + 'export-sller-products-stage', {seller_id: $scope.seller.id, stage: 'for_pricing'})
                .then(function (response)
                {
    ...
    $scope.generateProductLabelReport = function () {
        $rootScope.loader = true;
        $http.post(site_settings.api_url + 'export-seller-products-labels', {seller_id: $scope.seller.id, stage: 'for_pricing'})
                .then(function (response)
                {
                    var fileName = response.data;
                    var a = document.createElement('a');
...
app.controller('ProductForStoragePricingAddController', function (product_quotation, $parse, $document, $timeout, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    $scope.product_quotation = {};
    ...
    if (product_quotation)
    {
        ...
        $http.post(site_settings.api_url + 'product_quotation/get_product_quotation', {id: product_quotation})
                .then(function (response)
                {
                    $rootScope.loader = false;
                    $scope.product_quotation = response.data;
                    if ($scope.product_quotation.storage_pricing != '' && $scope.product_quotation.storage_pricing != null)
                    {
                        $scope.action = 'Edit';
                    }
                })
    ...
    $scope.saveProductForStoragePricing = function ()
    {
        $http.post(site_settings.api_url + 'product_for_pricing/save_product_storage_pricing', $scope.product_quotation)
                .then(function (response)
                {
...
app.controller('ProductForPricingViewController', function (product_quotation, $timeout, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    $scope.product = {};
...
app.controller('AssignAgentToProductQuotationController', function (products, $parse, $document, $timeout, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
