...
app.controller('DesignerConsignmentAgreementController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
...
//        console.log(newVal);
    ...
    if ($stateParams.product_quote_agreement_id)
    {
        ...
        $http.post(site_settings.api_url + 'designer_consignment_agreement/check_designer_consignment_agreement', {'product_quote_agreement_id': $stateParams.product_quote_agreement_id})
...
//                            $state.go('login');
                        $state.go('designer_consignment_agreement_completed', {status: 'already_submitted'});
                    }
...
//                        $rootScope.$emit("notification");
//                        $state.go('login');
                    $state.go('designer_consignment_agreement_completed', {status: 'invalid'});
    ...
    if ($stateParams.product_quote_agreement_id)
    {
        $scope.SaveDesignerConsignmentAgreement = function (is_valid)
        {
            if (is_valid)
            {
...
//                }
                ...
                $rootScope.loader = true;
                $http.post(site_settings.api_url + 'designer_consignment_agreement/save_designer_consignment_agreement', {'seller_ageement': $scope.seller_agreement, 'signature': $scope.signature})
                    .then(function (response)
                    {
                        $scope.response = response.data;
...
//                            $rootScope.$emit("notification");
//                            $state.go('login');
                        $state.go('designer_consignment_agreement_completed', {status: 'completed'});
                        $rootScope.loader = false;
                    }).catch(function (error)
                {
                    console.log(error);
...
app.controller('DesignerConsignmentAgreementCompletedController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    $scope.title = '';
    $scope.message = '';
