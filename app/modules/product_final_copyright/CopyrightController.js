...
app.controller('CopyrightController', function ($document, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder, ngAAToken, $window)
{
    $scope.seller = {};
...
//                    console.log($scope.user);
    ...
    $scope.dtOptions = DTOptionsBuilder.newOptions().withOption('ajax', {
        dataSrc: "data",
        url: site_settings.api_url + 'copyright/get_copyrights',
        type: "POST",
...
//        action_btn += '<md-button aria-label="VIEW" ng-click="openCopyrightViewDialog(' + data.id + ');" class="md-fab md-accent md-raised md-mini">';
...
//        action_btn += '<span ng-click="openCopyrightViewDialog(' + data.id + ');" class="text-boxed m-0 deep-blue-bg white-fg" style="cursor: pointer; background-color: #1eaa36 !important; margin-left: 5px;">VIEW</span>';
...
//        return '<md-button aria-label="Edit" ui-sref="edit_profile({id:\'' + full.id + '\'})" class="md-fab md-mini md-button  md-ink-ripple">  <md-icon md-svg-src="assets/img/edit.svg" class="icon"></md-icon></md-button><md-button aria-label="Delete" type="submit" class="md-fab md-mini md-button  md-ink-ripple md-warn">  <md-icon md-font-icon="icon-delete" class="icon" ></md-icon></md-button>';
    ...
    function agingRender(data, type, full, meta)
    {
        if (full.approved_created_at != null)
        {
            ...
            var startDate = moment.utc(full.copyright_created_at.date).local();
            var endDate = moment.utc(full.approved_created_at.date).local();
        ...
        {
            if (full.copyright_created_at != null)
            {
                var startDate = moment.utc(full.copyright_created_at.date).local();
...
//        DTColumnBuilder.newColumn('tlv_suggested_price_max').withTitle('Suggested Price'),
        DTColumnBuilder.newColumn('copyright_created_at').withTitle('Date').renderWith(renderDate),
...
//        console.log(full);
        if (full.status_id == 17)
        {
            ...
            action_btn += '<md-button aria-label="Approve" sglclick="changeCopyrightStatus(' + full.id + ',18);" class="md-fab md-accent md-raised md-mini">';
            ...
            action_btn += '<md-button aria-label="Reject" sglclick="changeCopyrightStatus(' + full.id + ',19);" class="md-fab md-warn md-raised md-mini">';
    ...
    $scope.changeProductStatus = function (status, product)
    {
        ...
        for (var i = 0; i < $scope.productStatus.length; i++)
        {
            ...
            {
                $scope.productStatus[i].copyright_status_id = status;
            } else
            {
                count++;
            }
        ...
        {
            $scope.productStatus.push({'copyright_status_id': status, 'product_quotation_id': product});
        }
//        $scope.productStatus.push({'copyright_status_id': status, 'product_quotation_id': product});
//        $scope.productStatus[product] = {'copyright_status_id': status, 'product_quotation_id': product};
    };
...
//        console.log(full);
        if (full.is_copyright == 0)
        {
            ...
            action_btn += '<md-radio-button value="1">Approve</md-radio-button>';
    ...
    $scope.changeCopyrightStatus = function ()
    {
        var rejected = [];
        for (var i in $scope.productStatus)
        {
            if ($scope.productStatus[i].copyright_status_id == 1)
            {
        ...
        if (rejected.length > 0)
        {
            ...
            $mdDialog.show(confirm).then(function ()
            {
                ...
                $http.post(site_settings.api_url + 'copyright/change_copyright_status', {product_status: rejected})
                        .then(function (response)
                        {
                            $scope.productStatus = [];
        ...
        {
            $rootScope.message = 'Please select at least one approve';
            $rootScope.$emit("notification");
        }
    };

    $scope.changeRejectCopyrightStatus = function ()
    {
        var rejected = [];
        for (var i in $scope.productStatus)
        {
            if ($scope.productStatus[i].copyright_status_id == 2)
            {
                rejected.push($scope.productStatus[i]);
            }
    ...
    $scope.DeleteSelectedProducts = function ()
    {
        ...
        for (var i in $scope.productStatus)
        {
            if ($scope.productStatus[i].copyright_status_id == 85)
            {
                rejected.push($scope.productStatus[i]);
            }
...
//                        $scope.downloadDocument();
    ...
    $scope.AllApproveProductSelected = function ()
    {
        $scope.productStatus = [];
        if ($scope.Approve)
        {
            ...
            {
                $scope.select[i] = 1;
                $scope.productStatus.push({'product_quotation_id': i, copyright_status_id: 1});
            }
        } else
        {
            $scope.Reject = false;
    ...
    $scope.AllRejectProductSelected = function ()
    {
        ...
        if ($scope.Reject)
        {
            ...
            {
                $scope.select[i] = 2;
                $scope.productStatus.push({'product_quotation_id': i, copyright_status_id: 2});
            }
        } else
        {
            $scope.Approve = false;
    ...
    $scope.AllDeleteProductSelected = function ()
    {
        ...
        if ($scope.Delete)
        {
            ...
            {
                $scope.select[i] = 85;
                $scope.productStatus.push({'product_quotation_id': i, copyright_status_id: 85});
            }
        } else
        {
            $scope.Approve = false;
...
//            });
    ...
    {
        $mdDialog.show({
            controller: 'CopyrightAddController',
            templateUrl: 'app/modules/product_final_copyright/views/copyright_add.html',
            parent: angular.element($document.body),
            clickOutsideToClose: true,
            locals: {
    ...
    $scope.openCopyrightViewDialog = function (product_quotation)
    {
        $mdDialog.show({
            controller: 'CopyrightViewController',
            templateUrl: 'app/modules/product_final_copyright/views/copyright_view.html',
            parent: angular.element($document.body),
            clickOutsideToClose: true,
            locals: {
                product_quotation: product_quotation
    ...
    $rootScope.$on("reloadCopyrightTable", function (event, args)
    {
//        $window.location.reload();
        $scope.select = [];
        reloadData();
...
app.controller('CopyrightAddController', function (product_quotation, $parse, $document, $timeout, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
...
//                    $rootScope.$emit("notification");
    ...
    $scope.getCat = function ()
    {
        ...
        {
            if ($scope.product_quotation.product_id[$scope.categorys[i].category_name.toLowerCase()] != null && $scope.categorys[i].category_name.toLowerCase() != 'look' && $scope.categorys[i].category_name.toLowerCase() != 'condition' && $scope.categorys[i].category_name.toLowerCase() != 'sub category' && $scope.categorys[i].category_name.toLowerCase() != 'condition' && $scope.categorys[i].category_name.toLowerCase() != 'room' && $scope.categorys[i].category_name.toLowerCase() != 'color' && $scope.categorys[i].category_name.toLowerCase() != 'collection')
            {
...
//            } 
//            else if ($scope.product_quotation.product_id['category'] != null && $scope.categorys[i].category_name == 'Sub Category')
//            {
...
//        }
        ...
        $scope.product_quotation.passfrom = 'copyright';
        ...
        $http.post(site_settings.api_url + 'product_final/save_product_quotation_final', $scope.product_quotation)
                .then(function (response)
                {
                    ...
                    $rootScope.$emit("notification");
//                    $rootScope.$emit("reloadUserTable");
                    $rootScope.$emit("reloadCopyrightTable");
                    $mdDialog.hide();
                }).catch(function (error)
        {
            $rootScope.message = 'Something Went Wrong';
...
app.controller('CopyrightViewController', function (product_quotation, $timeout, $mdDialog, $rootScope, $state, $compile, $resource, $scope, $auth, $q, $stateParams, $http, site_settings, DTColumnDefBuilder, DTOptionsBuilder, DTColumnBuilder)
{
    $scope.product = {};
